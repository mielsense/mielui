<script module lang="ts">
    ...
    type HugeiconData = readonly (readonly [string, Readonly<Record<string, string | number>>])[];
...
<script lang="ts">
    ...
    const resolvedStrokeWidth = $derived(
        requestedStrokeWidth === undefined || requestedStrokeWidth === null
            ? undefined
            : absoluteStrokeWidth && Number.isFinite(Number(size)) && Number(size) > 0
    ...
    function renderNodes(
        ...
        for (const [tag, sourceAttributes] of data) {
            ...
            if (consumerFill !== undefined && consumerFill !== null) {
                attributes.fill = consumerFill;
            }
            ...
            if (consumerStrokeWidth !== undefined) {
                attributes['stroke-width'] = consumerStrokeWidth;
            }
