...
utilities in Mielui components. Every bordered surface must resolve its width
from the token:
...
transparent). Dialog exits travel upward by the modal offset and shrink a
 ...
 Composer.Root renders `mielui-modal-frame` with the dense
 `--mielui-modal-inset: calc(var(--spacing) * 0.5)` Popover/Sheet use, not a
 hand-built bordered card. Composer.Input carries `mielui-inset-surface` (the
 ...
 giving it a second surface: the toolbar takes `mielui-inset-surface` with
 `rounded-t-none` and a negative top margin equal to
 `--mielui-modal-inset` to swallow the frame gap, and it flips an internal
