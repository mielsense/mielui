...
- `main` is the default and production branch. Keep it ready to deploy.
- Start each change from an up-to-date `main` on a short-lived branch named
  `feat/<description>`, `fix/<description>`, or `chore/<description>`.
- Do not create a permanent `develop`, staging, or release branch unless the
  user requests one.
- Use `type(scope): description` for every commit and pull request title.
...
polish, or dependency bumps that do not change how agents use Mielui.
