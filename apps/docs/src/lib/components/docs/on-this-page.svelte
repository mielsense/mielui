            ...
            <div bind:this={list} class="relative flex flex-col gap-0.5">
                ...
                {#each headings as heading (heading.id)}
                    <a
                        ...
                        onblur={() => { focused = null; }}
                        class={`${heading.id === 'api-reference' ? 'mt-2' : ''} relative rounded-md py-1.5 pr-1 text-sm leading-5 transition-colors motion-reduce:transition-none focus-visible:outline-2 focus-visible:outline-primary ${heading.level === 3 ? 'pl-9' : 'pl-6'} ${active === heading.id ? 'text-foreground' : 'text-foreground-muted hover:text-foreground'}`}
                    >
                        {heading.label}
                    </a>
                {/each}
