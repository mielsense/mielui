...
for (const orientation of ['horizontal', 'vertical'] as const) {
    it.each([false, true])(
        `keeps fractional ${orientation} rail padding balanced through keyboard selection and press scale (fixed fractional width: %s)`,
        async (fractionalWidth) => {
            render(TabsGeometryFixture, { orientation, fractionalWidth });
            ...
            // Chromium quantizes positions and sizes to 1/64px. The gap difference
            // includes two position errors and one size error before ancestor scaling.
            const layoutRoundingTolerance = 3 / 64;
            expect(Math.abs(leading - trailing)).toBeLessThan(layoutRoundingTolerance);
