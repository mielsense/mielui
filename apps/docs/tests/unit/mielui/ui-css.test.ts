...
const css = readFileSync(resolve(process.cwd(), '../../packages/mielui/src/ui.css'), 'utf8');
...
describe('ui.css Tier 1 primitives', () => {
    it('defines only the neutral steps consumed by components', () => {
        ...
        for (const step of [25, 200, 400, 600, 700, 800]) {
            expect(css).not.toContain(`--mielui-neutral-${step}:`);
        }
    });
...
describe('ui.css typography tokens', () => {
    ...
    it('defines the press motion contract', () => {
        ...
        expect(css).toContain('--spacing: var(--mielui-space-unit);');
        ...
        expect(css).toContain('.mielui-press:active');
    });
...
describe('ui.css Tier 3 + structure', () => {
    ...
    it('sizes controls from the spacing scale', () => {
        expect(css).toMatch(/--size-control-md:.*var\(--mielui-space-/);
    });
    ...
    it('declares the shared surface contracts in the components layer', () => {
        ...
        for (const contract of [
            ...
            '.mielui-modal-frame',
            '.mielui-inset-frame',
            '.mielui-inset-surface',
            '.mielui-tooltip',
            '.mielui-overlay-scrim',
            '.mielui-menu-label',
            '.mielui-menu-separator',
            '.mielui-error-notice'
        ]) {
