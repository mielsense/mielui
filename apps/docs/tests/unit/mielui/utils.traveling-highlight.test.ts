...
afterEach(() => {
    ...
    document.documentElement.style.removeProperty('--mielui-traveling-highlight');
    vi.unstubAllGlobals();
});
...
describe('travelingHighlight', () => {
    it('prefers the transient active item over an earlier selected item', async () => {
        ...
        await settle();
        expect(
            surface.querySelector<HTMLElement>('.mielui-item-highlight')?.style.transform
        ).toContain('40px');
        action.destroy?.();
    });
    ...
    it('still mounts on a coarse-pointer device', async () => {
        ...
        expect(surface.querySelector('.mielui-item-highlight')).not.toBeNull();
        expect(surface.classList.contains('mielui-collection-surface')).toBe(true);
        action.destroy?.();
    });
    ...
    it('keeps a nested collection from moving its parent highlight', async () => {
        ...
        await settle();
        expect(
            parent.querySelector<HTMLElement>(':scope > .mielui-item-highlight')?.style.transform
        ).toContain('10px');
    ...
    it('still highlights the active item when traveling is off', async () => {
        document.documentElement.style.setProperty('--mielui-traveling-highlight', 'none');
        const surface = document.createElement('div');
        const selected = document.createElement('button');
        const active = document.createElement('button');
        selected.dataset.collectionItem = '';
        ...
        const action = travelingHighlight(surface);
        await settle();
        const highlight = surface.querySelector<HTMLElement>('.mielui-item-highlight');
