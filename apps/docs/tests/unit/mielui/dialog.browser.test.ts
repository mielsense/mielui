import { dialogIn, dialogOut } from '@mielui/svelte/transition';
...
/*
 * Browser-mode tests for dialog. These exercise mount/unmount, focus
...
describe('Dialog -- open/closed mount in real browser', () => {
    ...
    it('renders Dialog.Content when open=true', async () => {
        ...
        await expect.element(page.getByText('Dialog Description')).toBeInTheDocument();
    });
...
describe('Dialog -- error browser chrome', () => {
    it('sets and restores theme-color while an error dialog is open', async () => {
        const themeColor = document.createElement('meta');
        themeColor.name = 'theme-color';
        themeColor.content = '#ffffff';
        document.head.append(themeColor);

        render(DialogFixture, { open: true, error: true });
        await flush();
...
describe('Dialog -- close paths actually unmount (P3-F6 disambiguation)', () => {
    ...
    it('unmounts content on click outside (when allowClickOutside is true)', async () => {
        render(DialogFixture, { open: true, allowClickOutside: true });
        await flush();
    ...
    it('does NOT unmount on click outside when allowClickOutside is false', async () => {
        render(DialogFixture, { open: true, allowClickOutside: false });
        await flush();
        await new Promise((r) => setTimeout(r, 20));
        await expect.element(page.getByText('Dialog Title')).toBeInTheDocument();
...
describe('Dialog -- ARIA contract in browser', () => {
    it('uses the vertical layout and compact width when requested', async () => {
        render(DialogFixture, { open: true, orientation: 'vertical' });
        await flush();
    ...
    it('honors role="alertdialog" prop', async () => {
        render(DialogFixture, { open: true, role: 'alertdialog' });
        await flush();
...
describe('Dialog -- body scroll lock in browser', () => {
    it('keeps the dialog content surface scrollable while the page is locked', async () => {
        render(DialogFixture, { open: true });
        await flush();
...
describe('Dialog -- focus management in browser', () => {
